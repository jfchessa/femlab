function [ke, re] = kmat_mindlin_quad4(gcoord, h, e, nu, qvect,...
    dir, rotzStiffFactor)
...
%
%
% [ke, re] = kmat_mindlin_quad4( gcoord, h, e, nu )
% [ke, re] = kmat_mindlin_quad4( gcoord, h, e, nu, qvect )
% [ke, re] = kmat_mindlin_quad4( gcoord, h, e, nu, qvect, dir )
%
%
%
...
elemArea = 0;
H = ShellDirectionCosines(gcoord);  % direction cosines of the element ( need to do this at teh center)
coord=gcoord;
...
for i=1:size(gShearLocs,1)
    ...
    s = gShearLocs(i, 1);   % quad points
	t = gShearLocs(i, 2); 
	w = gShearWts(i);       % quad weights
	
    n = [(1/4)*(1 - s)*(1 - t), (1/4)*(s + 1)*(1 -t), ...
        (1/4)*(s + 1)*(t + 1), (1/4)*(1 - s)*(t + 1)];  % shape functions

    dns=[(-1 + t)/4, (1 - t)/4, (1 + t)/4, (-1 - t)/4];  % shape function derivaticves
